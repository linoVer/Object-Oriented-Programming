		...
		for (int i = 0; i < listaShow.size(); i++) {		
			 ...
			 for (int j = 0; j < listaShow.get(i).getArtista().getListaTitulo().size(); j++) {
				 ...
				 if(listaShow.get(i).getArtista().getListaTitulo().get(j).getDescricao() != str){
					 str +=("Descrição : "+ listaShow.get(i).getArtista().getListaTitulo().get(j).getDescricao() +"\n");
					 str += ("\n\n");
			 ...
			 JOptionPane.showMessageDialog(null, str, "Lista de Títulos do show",JOptionPane.INFORMATION_MESSAGE);
		}	
